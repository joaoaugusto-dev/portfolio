...
router.get("/", async (req, res) => {
  const items = await SocialLink.findAll({ order: [["order", "ASC"]] });
  res.json(items);
});
...
router.put("/reorder", requireAuth, async (req, res) => {
  ...
  await sequelize.transaction((t) =>
    Promise.all(ids.map((id, order) => SocialLink.update({ order }, { where: { id }, transaction: t }))),
  );
  res.json(await SocialLink.findAll({ order: [["order", "ASC"]] }));
});
